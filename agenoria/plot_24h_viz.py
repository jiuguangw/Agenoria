...
from config import diaper_data, feeding_bottle_data, feeding_solid_data
from config import param as config
from config import sleep_data

from .plot_settings import (export_figure, format_24h_week_plot_horizontal,
...
def plot_sleep_24h_viz() -> None:
    ...
    data.loc[index].apply(lambda row: fig_ax.broken_barh([(row[
        'day_number'] + 1, BAR_SIZE)], [0, row['offset']]),
                          axis=1)

    # Loop through each row and plot the duration
    data.apply(lambda row: fig_ax.broken_barh([(row[
        'day_number'], BAR_SIZE)], [row['timestamp_hour'], row['duration']]),
               axis=1)

    # End date - one year or full
    end_date = get_end_date(data['day_number'],
...
def plot_feeding_24h_viz() -> None:
    ...
    # Plot
    fig_ax.scatter(data_bottle['day_number'],
                   data_bottle['timestamp_hour'],
                   s=25,
                   c='r')
    fig_ax.scatter(data_solid['day_number'] + offset,
    ...
    # End date - one year or full
    end_date = get_end_date(data_bottle['day_number'],
                            config['output_format']["output_year_one_only"])
...
def plot_diapers_24h_viz() -> None:
    ...
    # Plot
    fig_ax.scatter(data['day_number'],
                   data['timestamp_hour'],
                   s=25,
                   c=data['Color key'])
    ...
    plt.legend(handles=[
        blue_patch, green_patch, brown_patch, red_patch, yellow_patch
    ])
