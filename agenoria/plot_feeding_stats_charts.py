...
def export_feeding_text(feeding_data: pd.DataFrame, key_amount: str,
                        label: str) -> None:
    print(label + ': ', end='')
...
def parse_glow_feeding_data(data: pd.DataFrame,
                            key_amount: str) -> pd.DataFrame:
    ...
    for current_date in pd.date_range(start_date, end_date):
        ...
        rows_on_date = rows_on_date.sort_values(['Time of feeding'],
                                                ascending=True)
        ...
        feeding_data_list.append([
            current_date, sum_on_date, mean_on_date, min_on_date, max_on_date,
            sessions_on_date, time_between_feeding_max,
            time_between_feeding_mean, time_between_feeding_min, daytime_sum,
            nighttime_sum, nighttime_feeding_count
        ])

    # Convert list to dataframe
    daily_data_new = pd.DataFrame(feeding_data_list,
                                  columns=[
                                      'date', 'sum', 'mean', 'min', 'max',
                                      'sessions', 'time gap max',
                                      'time gap mean', 'time gap min',
...
def combine_bottle_solid(data_bottle: pd.DataFrame,
                         data_solid: pd.DataFrame) -> pd.DataFrame:
    ...
    solid_new = pd.concat([zero_data['sum'], data_solid['sum']],
                          axis=0,
                          ignore_index=True)
...
def plot_feeding_stats_charts() -> None:
    ...
    mmm_plot(axarr[0, 0], data_bottle['date'], data_bottle['mean'],
             data_bottle['min'], data_bottle['max'])
    ...
    mmm_plot(axarr[2, 2], data_bottle['date'], data_bottle['time gap mean'],
             data_bottle['time gap min'], data_bottle['time gap max'])
