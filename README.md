...
### Installing Pixi (Recommended)

This project uses [Pixi](https://pixi.sh/latest/) for environment and task
management.
...
From the repository root, install and lock the environment:

```bash
pixi install
```

To work inside an activated shell:
...
pixi run agenoria config/config_zyw.toml
```
If for some reason you'd rather skip `setuptools`, you can launch the module directly with
```bash
# Pass in the configuration file in .toml
pixi run python -m agenoria config/config_zyw.toml
```
